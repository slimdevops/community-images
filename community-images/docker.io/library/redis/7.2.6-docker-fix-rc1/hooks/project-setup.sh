...
if [[ -z "$digest" ]]; then
  json_latest_tags=$(curl -s https://hub.docker.com/v2/repositories/focaloiddev/redis/tags/7.2.6-docker-fix-rc1)
...
json_tags=$(
  curl -s https://hub.docker.com/v2/repositories/focaloiddev/redis/tags?page_size=100 &
  page=1
)
