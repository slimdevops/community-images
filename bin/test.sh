...
if [ "$PROJECT_TEST_SUITE" == "custom" ]; then
  ...
  exec "docker run --rm $PROJECT_TEST_SUITE_HOST_ARGS --net=host -v $(pwd)/cypress-e2e:/e2e -v $(pwd)/results/$ENV_TEST:/e2e/results:rw -w /e2e -e CYPRESS_VIDEO=false cypress/included:12.3.0"
else
