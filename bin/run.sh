...
if [[ $STAGE == "FAT" ]]; then
    if [[ $PROJECT_SOURCE == "dockerfile" ]]; then
        IMAGE="$SLIM_IMAGE_REGISTRY/$PROJECT_TARGET_IMAGE"
    else
        IMAGE=$PROJECT_ORIGINAL_IMAGE
    fi
elif [[ $STAGE == "INSTRUMENT" ]]; then
    HOST_ARGS="$HOST_ARGS --cap-add ALL $RUN_INSTRUMENTED_AS_USER"
    IMAGE="$SLIM_IMAGE_REGISTRY/$PROJECT_FULL_IMAGE_INSTRUMENTED"

elif [[ $STAGE == "SLIM" ]]; then
    IMAGE="$SLIM_IMAGE_REGISTRY/$PROJECT_FULL_IMAGE_SLIMMED"
fi
